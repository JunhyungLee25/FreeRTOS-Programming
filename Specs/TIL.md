...
### 2026-02-09
[시스템 성능 최적화 기법](./System_Performance_Optimization.md)

- 우선순위 배정 방법: 각 태스크 단위의 기능으로 우선순위를 결정하면 안된다.
	모든 값을 수치화한 후 결정한다. 우선순위를 객관적으로 설정 가능하다.
	![](../images/Pasted_image_20260209154412.png)
	- 각 TASK 별로 임계시간 혹은 **마감시간**을 측정 및 예측하여 수치화한다. 이를 통해 고정된 우선순위를 사용할 수 있다.
	- 예를 들어, 음악재생은 오디오 버퍼로 디코딩할때 최대 30ms 안에 계속해서 수행되어야 하므로 30ms이고, 사용자 입력은 키를 누른 후 700ms 이내에 반응하면 불을 느끼지 않을 시간으로 설정하였다. 따라서 마감시간이 가장 짧은 음악재생 태스크가 가장 높은 우선순위를 부여받았다.
	- 마감시간에 맞추어 우선순위를 정한 후 동작시켰을 때, 마감시간 내에 들어오지 못하는 태스크가 있다면 그에 대해 분석을 하면 된다.(우선순위 변경하면 안됨!)
