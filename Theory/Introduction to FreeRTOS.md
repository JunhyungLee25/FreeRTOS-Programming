# RTOS란
>여러 태스크를 동시에 처리하는 멀티태스킹 능력과, 어떤 상황에서도 특정 작업이 정해진 시간안에 완료됨을 보장하는 실시간성을 제공하는 OS이다.
...
running: execute
<br>blocked: not execute. sleep=waiting.  
...
- 선점형 스케쥴링의 특성을 부여받음.
	...
	우선순위가 높은 task가 생기면 무조건 cpu를 점유함 -> 우선순위가 가장 높은 task가 cpu를 계속 점유하면 다른 task는 동작하지 못하는데 이를 starvation(기아) 현상이라 한다.
****
## 인터럽트
...
<br>Blocking 단점: 함수실행 후 휴면상태에 들어가면 아무 코드도 실행할 수 없다.

Non-Blocking 장점: 휴면상태에 들어가지 않고 loop 계속 실행 가능
<br>Non-Blocking 단점: 휴면상태로 들어가 기다리지 않고 계속 함수 실행하여 확인해야한다.
****
## 커널의 시계(TICK)
>정기적(Periodic)으로 일어나는 인터럽트

- task 지연(delayed), 타임아웃(timeout)을 제공한다.
- task 지연의 분해능(clock resolution)은 클럭 틱(TICK) 하나
